...
*******************************************************************
  ...
  !Lines with "Javier aguilar sanchez modified this function ***" Are new lines that modify the original work
  !by Richard Chandler that adapt his routines to my (javier aguilar sanchez) goals and taste.
...
*
      ...
      IF (.NOT.( (P.GE.0.0D0).AND.(P.LE.1.0D0) )) THEN
       ...
       RETURN
      ELSEIF(N.LE.0) THEN
       if ( N.lt.0) then !Javier aguilar sanchez modified this function ***
         WRITE(*,2)  !So I just receive error message if N<0. If N=0 output is not random and ZBQLBIN(0,anything)=0
       end if !***
       !WRITE(*,1) !Originial line ***
       RETURN
      ENDIF
*
...
*
 ...
 1    FORMAT(/5X,'****ERROR**** Illegal parameter p value in ',
     +' ZBQLBIN',/)

 2    FORMAT(/5X,'****ERROR**** Illegal parameter N value in ', !Javier aguilar sanchez modified this function ***
     +' ZBQLBIN',/) !***
